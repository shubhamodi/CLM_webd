...
export const getStaticPathBlogAndTag = async ({ paginate }) => {
  if (!isBlogEnabled || !isBlogPostRouteEnabled) return [];
  ...
  const tagPaths = Array.from(tags).flatMap((tag) =>
    paginate(
      posts.filter((post) => Array.isArray(post.tags) && post.tags.find((elem) => elem.toLowerCase() === tag)),
      {
        params: { tag: tag, blog: TAG_BASE || undefined },
  ...
  const postPaths = posts.flatMap((post) => ({
    params: {
      blog: post.permalink,
    },
    props: { post },
  ...
  return [...tagPaths, ...postPaths];
};


export const getStaticBlogsAndTags = async ({ paginate }) => {
  if (!isBlogEnabled || !isBlogPostRouteEnabled) return [];
  ...
  paths = Array.from(tags).flatMap((tag) =>
    paginate(
      posts.filter((post) => Array.isArray(post.tags) && post.tags.find((elem) => elem.toLowerCase() === tag)),
      {
        params: { tag: tag, blog: TAG_BASE || undefined },
